...
@section('main')
			...
			<h3>{{ date('F j, Y', strtotime($Editorial->created_at)) }}</h3>
			...
			<div class="article" style="width:96%; margin:0 auto; background-color:#fff; border:1px solid #cfcfcf; border-radius:15px;">
				...
				<div class="articleimg" style="height:400px; width:calc(40% - 20px); padding-left:20px; float:left;">
					<img style="height:100%; width:100%;" src="images/{{ $Editorial->image }}" />
				</div>
				
				<div class="articleinfo" style="width:calc(60% - 60px); padding-left:30px; padding-right:30px; float:left; white-space:pre-wrap;">{{ languages($Editorial->editorial, $Editorial->editorial_fr) }}</div>				
			</div>
